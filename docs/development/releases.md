...
2026-09-19 기준 업데이트 key pair와 GitHub Actions의 signing secret·공개키 variable은 등록했습니다. 최초 공개 릴리스와 두 OS의 실제 N→N+1 설치는 아직 검증 전입니다. 키 등록만으로 업데이트가 운영 중이라고 표시하지 않습니다. 키가 없는 별도 개발 빌드는 업데이트 확인 대신 설정되지 않았다는 상태를 표시합니다.
