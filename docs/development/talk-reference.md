...
화자·표정 뒤에 선택적인 모션을 붙일 수 있다. 생략하면 캐릭터의 기본 모션 설정을 따른다.

```text
A[기쁨]{motion=wave}: 한 번 손을 흔들어.
A{motion="손 흔들기" repeat=true interval=2s}: 이 대사가 끝날 때까지 반복해.
B{motion=none}: 이 줄에서는 정지 상태로 말해.
A{motion=inherit}: 캐릭터의 기본 설정을 따라.
```

`motion` 값은 해당 화자에게 등록된 클립 ID이며, 이름이 아니다. `repeat`는 `true`·`false`이고 기본값은 `false`, `interval`은 기존 시간 단위 표기의 `0`~`60000ms`이고 기본값은 `0`이다. 클립 반복은 해당 대사가 끝나면 종료된다. 공백·특수문자가 있는 ID는 JSON 큰따옴표 문자열로 쓴다. 따옴표 없는 `none`은 정지, `inherit`는 기본 설정이며 두 값에는 반복 옵션을 붙이지 않는다. 실제 ID가 `none` 또는 `inherit`라면 따옴표로 감싼다. 옵션 중복·알 수 없는 옵션·범위를 벗어난 값은 `MOTION` 진단으로 거부한다.

CLI의 `check`·`simulate`는 모션 문법을 검사하며 클립 소유자 검사는 앱 재생 경로에서 수행한다. 앱은 실제 화자가 정해진 뒤 없는 클립을 참조하는 장면만 후보에서 제외하고 다른 장면은 계속 선택한다. 이때 로그의 후보 사유는 `motion_error`다. 모션 표기는 콜론 앞에만 적용되므로 본문에 적힌 `{motion=wave}`는 문자 그대로 남는다.
