...
export function WordbookPanel({
  ...
  return (
    <section className={s.section} aria-label="단어장">
      <h2 className={s.sectionTitle}>단어장</h2>
      <p className={s.quiet}>
        키워드가 포함되면 등록한 대사를 그대로 재생해요. 긴 키워드를 우선하고 길이가 같으면 먼저
        등록한 항목을 사용해요.
      </p>
      <div className={w.entries} aria-label="단어장 항목 목록">
        {list.map((draft) => (
          <button
            key={draft.entry.id}
            type="button"
      ...
      >
        <fieldset className={w.editor} disabled={pending}>
          ...
          <p className={s.quiet}>쉼표나 줄바꿈으로 키워드를 나눠 주세요.</p>
          <div className={s.row}>
          ...
          <div className={w.actions}>
            <button type="submit" className={s.primary} disabled={!valid || !current.dirty}>
              {pending ? "처리 중…" : "단어장 저장"}
            </button>
            {current.persisted && (
              <button type="button" className={s.button} onClick={() => void remove()}>
                항목 삭제
              </button>
            )}
          </div>
