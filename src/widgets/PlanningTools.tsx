import { FormField, Button, DateField, Select, TextArea, TextField } from "@fleetia/lagrange";
...
export function MemoTool({ widget, act }: Props): ReactElement {
  const notes = rows(record(widget.data).notes);
  const titleInput = useRef<HTMLInputElement>(null);
  const [editorOpen, setEditorOpen] = useState(notes.length === 0);
  ...
  return (
    <>
      ...
      {notes.length === 0 && <p className={c.quiet}>아직 메모가 없어요. 첫 메모를 남겨 보세요.</p>}
      {notes.map((note) => (
        <article className={s.item} key={text(note.id)}>
          <h2 className={s.sectionTitle}>{text(note.title)}</h2>
          <p className={s.prose}>{text(note.body)}</p>
          <div className={s.row}>
            <Button
              variant="secondary"
              onClick={() => {
                ...
                setBody(text(note.body));
              }}
            >
              수정
            </Button>
            <Button variant="secondary" onClick={() => void act("delete", { id: text(note.id) })}>
              삭제
            </Button>
          </div>
        </article>
      ...
      {editorOpen && (
        <form
          ...
          onSubmit={async (e) => {
            e.preventDefault();
            if (await act(id ? "update" : "add", id ? { id, title, body } : { title, body })) {
              closeEditor();
            }
          }}
        >
          <h2 className={s.sectionTitle}>{id ? "메모 수정" : "새 메모"}</h2>
          <FormField className={c.field} label="메모 제목" required>
            <TextField
              ref={titleInput}
              maxLength={500}
              value={title}
          ...
          <FormField className={c.field} label="메모 본문">
            <TextArea maxLength={50000} value={body} onChange={(e) => setBody(e.target.value)} />
          </FormField>
          <div className={s.actions}>
            <Button type="submit" variant="primary">
              {id ? "메모 변경 저장" : "메모 추가"}
